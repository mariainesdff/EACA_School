...
`cases'` is a general purporse tactic to deconstruct hypotheses. Here we present two of the most
basic use cases.


### Examples

1) Given the hypothesis `hPaQ : P ∧ Q`, the instruction `cases' hPaQ with hP hQ` will delete
...
2) Given the hypothesis `hPiQ : P ↔ Q`, `cases' hPiQ with hPQ hQP` will replace `hPiQ` by the 
two hypotheses
```
hPQ : P → Q
hQP : Q → P
